...
const Level4Screen = ({ onLevelComplete }) => {
  const [grid, setGrid] = useState([]);
  const [words, setWords] = useState([
    { word: 'FOOTBALL', found: false },
    { word: 'BASKETBALL', found: false },
    { word: 'TENNIS', found: false },
    { word: 'SOCCER', found: false },
    { word: 'HOCKEY', found: false },
    { word: 'GOLF', found: false }
  ]);
  ...
  useEffect(() => {
    ...
    const { grid: generatedGrid } = generateWordGrid(
      words,
      GRID_SIZE,
      'hard' // Hard difficulty for level 4
    );
  ...
  return (
    <LevelContainer backgroundStyle="sports">
      <LevelHeader>
        <LevelTitle>Level 4 - Sports</LevelTitle>
      ...
      {isLevelComplete && (
        <LevelCompleteOverlay>
          <LevelCompleteCard>
            <CompleteTitle>Level 4 Complete!</CompleteTitle>
            <CompleteMessage>You found all the sports!</CompleteMessage>
...
const LevelContainer = styled.div`
  ...
  background: ${props => {
    switch (props.backgroundStyle) {
      case 'sports':
        return 'linear-gradient(135deg, #f9fef3 0%, #e8f5e9 100%)';
      default:
