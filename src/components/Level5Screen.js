...
const Level5Screen = ({ onLevelComplete }) => {
  const [grid, setGrid] = useState([]);
  const [words, setWords] = useState([
    { word: 'JAVASCRIPT', found: false },
    { word: 'PYTHON', found: false },
    { word: 'JAVA', found: false },
    { word: 'RUBY', found: false },
    { word: 'SWIFT', found: false },
    { word: 'KOTLIN', found: false },
    { word: 'RUST', found: false }
  ]);
  ...
  useEffect(() => {
    ...
    const { grid: generatedGrid } = generateWordGrid(
      words,
      GRID_SIZE,
      'hard' // Hard difficulty for level 5
    );
  ...
  return (
    <LevelContainer backgroundStyle="programming">
      <LevelHeader>
        <LevelTitle>Level 5 - Programming Languages</LevelTitle>
      ...
      {isLevelComplete && (
        <LevelCompleteOverlay>
          <LevelCompleteCard>
            <CompleteTitle>Level 5 Complete!</CompleteTitle>
            <CompleteMessage>You found all the programming languages!</CompleteMessage>
...
const LevelContainer = styled.div`
  ...
  background: ${props => {
    switch (props.backgroundStyle) {
      case 'programming':
        return 'linear-gradient(135deg, #f0f4ff 0%, #e2e7fd 100%)';
      default:
